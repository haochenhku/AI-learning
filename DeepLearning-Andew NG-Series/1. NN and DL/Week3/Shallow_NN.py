...
def backward_propagation(parameters, cache, X,Y):
    ...
    dZ2 = (A2-Y) # (n_y,m)
    dW2 = np.dot(dZ2, A1.T)/m # (n_y,m) (m,n_h)->(n_y,n_h)
    db2 = np.sum(dZ2,axis=1,keepdims=True)/m
    dZ1 = np.dot(W2.T, dZ2) * (1-np.power(A1,2)) # (n_h,n_y)*(n_y,m)=(n_h,m).(n_h,m)
    dW1 = np.dot(dZ1, X.T)/m # (n_h,m)*(m,2)->(n_h,2)
