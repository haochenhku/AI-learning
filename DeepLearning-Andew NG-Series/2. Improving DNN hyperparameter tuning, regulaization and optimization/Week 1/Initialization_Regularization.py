...
def initialize_parameter_appraoch(layers_dims, approach='he'):
    """
    Arguments:
    layer_dims -- python array (list) containing the size of each layer.
    appraoch -- initialize appraoches
...
def model(X,Y,learning_rate=0.3, num_iterations = 30000,print_cost=True,lambd=0, keep_prob=1):
    ...
    parameters = initialize_parameter_appraoch(layers_dims,'he')

    # loop for gradiend descent
    for i in range(0, num_iterations):
        ...
        if print_cost and i%10000 == 0:
            print("Cost after ieration {}: {}".format(i,cost))
...
def compute_cost_with_regularization(A3,Y,parameters, lambd):
    ...
    L2_regularization_csot =  (np.sum(np.square(W1))+np.sum(np.square(W2))+np.sum(np.square(W3)))*lambd/(2*m)

    return cross_entropy_cost + L2_regularization_csot
...
def forward_propagation_with_dropout(X, parameters, keep_prob = 0.5):
    ...
    Z1 = np.dot(W1, X) + b1
    A1 = relu(Z1) # (n1,n_in)*(n_in,m)=(n_in, m)
